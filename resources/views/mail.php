...
function sendCMail($to, $subject, $template_name, $message, $headers, $other=array()){  
    ...
    if(@$to != ""){
        if(ISSMTP == 0) {
            ...
            $mail->Username = 'test@khalaf.imakeawesomethings.com';
            //Password to use for SMTP authentication
            $mail->Password = '$~3XXY!MC#{Q';
            //Set who the message is to be sent from
            $mail->setFrom('test@khalaf.imakeawesomethings.com', 'khalaf');
            //Set an alternative reply-to address
            $mail->addReplyTo('test@khalaf.imakeawesomethings.com', 'khalaf');
...
function sendForgetMail($to, $subject, $template_name, $message, $headers, $other=array()){  
    if($to != ""){
        ...
        {
            $setting_query2 = $db->prepare("SELECT * FROM `Resignation` WHERE id=:id");
